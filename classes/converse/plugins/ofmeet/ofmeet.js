(function (root, factory) {
    ...
    converse.plugins.add("ofmeet", {
        ...
        'initialize': function () {
            ...
            _converse.api.settings.update({
                ofmeet_modal: false,
                ofmeet_url: 'https://pade.yoru.ba',
                ofmeet_confirm: "Meeting?",
                ofmeet_invitation: 'Please join meeting in room at'
            });
            ...
            MeetDialog = _converse.BootstrapModal.extend({
                ...
                toHTML() {
                  ...
                  return '<div class="modal" id="myModal"> <div class="modal-dialog modal-lg"> <div class="modal-content">' +
                         ...
                         '</div>' +
                         '<div style="text-align: center;" class="modal-body"><iframe src="' + url + '" id="ofmeet" allow="microphone; camera;" frameborder="0" seamless="seamless" allowfullscreen="true" scrolling="no" style="z-index: 2147483647;width:640px;height:480px;display: inline-block"></iframe></div>' +
            ...
            console.log("ofmeet plugin is ready");
        },
        ...
        'overrides': {
            'onConnected': function () {
                ...
                _converse.on('message', function (data)
                {
                    ...
                    if (bodyElement && _converse.shouldNotifyOfMessage(data.stanza))
                    {
                        ...
                        var url = _converse.api.settings.get("ofmeet_url");
                        var pos = body.indexOf(url + "/");

                        if (pos > -1)
                        {
                            var room = body.substring(pos + url.length + 1);
                            var label = pos > 0 ? body.substring(0, pos) : _converse.api.settings.get("ofmeet_invitation");
            ...
            ChatBoxView: {
                ...
                renderToolbar: function renderToolbar(toolbar, options) {
                    ...
                    addToolbarItem(view, id, "pade-ofmeet-" + id, '<a class="fas fa-video" title="Jitsi Meet"></a>');

                    setTimeout(function()
                    {
                        var jitsiMeet = document.getElementById("pade-ofmeet-" + id);

                        if (jitsiMeet) jitsiMeet.addEventListener('click', function(evt)
                        {
                            evt.stopPropagation();

                            var jitsiConfirm = _converse.api.settings.get("ofmeet_confirm");
            ...
            MessageView: {
                ...
                renderChatMessage: async function renderChatMessage()
                {
                    ...
                    if (pos > -1)
                    {
                        ...
                        var link_label = pos > 0 ? body.substring(0, pos) : _converse.api.settings.get("ofmeet_invitation");
    ...
    var setupContentHandler = function(chat, avRoom, content, linkId, linkLabel)
    {
        ...
        if (avRoom && linkId)
        {
            setTimeout(function()
            {
                ...
                {
                    var url = _converse.api.settings.get("ofmeet_url") + '/' + avRoom;
    ...
    var doVideo = function doVideo(view)
    {
        ...
        var url = _converse.api.settings.get("ofmeet_url") + '/' + room;

        console.debug("doVideo", room, url, view);

        var label = _converse.api.settings.get("ofmeet_invitation");
    ...
    var doLocalVideo = function doLocalVideo(view, room, url, label)
    {
        ...
        var modal = _converse.api.settings.get("ofmeet_modal") == true;
        ...
        if (modal)
        {
            ...
            if (div)
            {
                div.innerHTML = '<iframe src="' + url + '" id="ofmeet" allow="microphone; camera;" frameborder="0" seamless="seamless" allowfullscreen="true" scrolling="no" style="z-index: 2147483647;width:100%;height:-webkit-fill-available;height:-moz-available;"></iframe>';

                var jitsiDiv = div.querySelector('#ofmeet');
                var firstTime = true;
                ...
                jitsiDiv.addEventListener("load", function ()
                {
                    ...
                    if (firstTime) firstTime = false;   // ignore when ofmeet room url is loaded
